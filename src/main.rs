...
pub mod models;
use dotenv::dotenv;
use models::{AppState, CallbackParams};
...
async fn get_oauth_token() -> Result<String> {
    ...
    run_local_server(csrf_token, client).await
}
...
async fn run_local_server(csrf_token: CsrfToken, client: BasicClient) -> Result<String> {
